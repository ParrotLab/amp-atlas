...
export function useFileWatchers(filePath: string | undefined, rootPath: string, enabled = true): Watcher[] {
  const [watchers, setWatchers] = useState<Watcher[]>([])
  const relPath = filePath && rootPath ? filePath.replace(rootPath + '/', '') : ''

  const refresh = useCallback(async () => {
    if (!relPath || !rootPath || !enabled) { setWatchers([]); return }
    const r = await window.api.git.fileWatchers(rootPath, relPath)
    const list = r.ok ? r.watchers : []
    cache.set(rootPath + '::' + relPath, list)
    setWatchers(list)
  }, [relPath, rootPath, enabled])

  useEffect(() => {
    // Only relevant on a Draft — on the read-only Live Version there's nothing to coordinate, and
    // we skip the GitHub call entirely (also avoids errors on systems that aren't connected).
    if (!relPath || !enabled) { setWatchers([]); return }
  ...
  }, [relPath, rootPath, enabled, refresh])
